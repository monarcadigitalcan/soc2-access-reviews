...
def phase1_trace_sources(service):
    ...
    with open(INPUT_CSV, encoding="utf-8") as infile:
        ...
        for row in reader:
            ...
            if total % 50 == 0:
                print(
                    f"[TRACE] {total}/603 checked | {len(sources)} unique sources found | {trace_errors} errors",
                    flush=True,
                )
    ...
    print(
        f"\n[TRACE COMPLETE] {len(sources)} unique source/email combos found from {total} files. Saved to {SOURCES_CSV}",
        flush=True,
    )
...
def phase2_revoke_sources(service, sources):
    ...
    with open(EVIDENCE_CSV, mode="w", newline="", encoding="utf-8") as outfile:
        writer = csv.DictWriter(
            outfile,
            fieldnames=["Timestamp", "Source Name", "Source ID", "Target Email", "Permission Type", "Action", "Reason"],
        )
        writer.writeheader()
        outfile.flush()
                ...
                writer.writerow(
                    {
                        ...
                        "Action": "SKIPPED",
                        "Reason": "Direct permission — retry individually",
                    }
                )
                outfile.flush()
                continue
