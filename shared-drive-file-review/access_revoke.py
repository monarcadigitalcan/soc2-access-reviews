...
with open("input/emails_revoke.txt") as f:
        ...
        while True:
            ...
            for file in results.get("files", []):
                try:
                    perms = (
                        service.permissions()
                        .list(fileId=file["id"], supportsAllDrives=True, fields="permissions(id, emailAddress)")
                        .execute()
                        .get("permissions", [])
                    )
                    ...
                    for p in perms:
                        ...
                        if email in [e.lower() for e in EMAILS_TO_REVOKE]:
                            ...
                            service.permissions().delete(
                                fileId=file["id"], permissionId=p["id"], supportsAllDrives=True
                            ).execute()
