...
def phase1b_trace_sources(service, samples):
    ...
    for (email, _perm_type), row in samples.items():
        checked += 1
        file_id = row.get("File ID", "").strip()
        ...
        if checked % 20 == 0:
            print(
                f"[TRACE] {checked}/{len(samples)} checked | {len(sources)} inherited sources | {len(direct_perms)} direct | {trace_errors} errors",
                flush=True,
            )
    ...
    print(
        f"\n[TRACE COMPLETE] {len(sources)} inherited sources + {len(direct_perms)} direct permissions from {checked} combos. Errors: {trace_errors}",
        flush=True,
    )
...
def phase2_revoke(service, sources, direct_perms):
    ...
    print(
        f"\n=== PHASE 2: REVOKING {total_targets} PERMISSIONS ({len(sources)} inherited + {len(direct_perms)} direct) ===",
        flush=True,
    )
    ...
    with open(EVIDENCE_CSV, mode="w", newline="", encoding="utf-8") as outfile:
        ...
        for _key, src in sources.items():
            ...
            try:
                results = (
                    service.permissions()
                    .list(
                        fileId=source_id, fields="permissions(id, emailAddress, domain, type)", supportsAllDrives=True
                    )
                    .execute()
                )
                ...
                outfile.flush()
                print(
                    f"[{timestamp}] REVOKED inherited {perm_type}:{target_email} from folder '{source_name}'",
                    flush=True,
                )
        ...
        for dp in direct_perms:
            ...
            try:
                service.permissions().delete(fileId=file_id, permissionId=perm_id, supportsAllDrives=True).execute()
                ...
                writer.writerow(
                    {
                        ...
                        "Target Email": target_email,
                        "Permission Type": dp["Permission Type"],
                        "Action": "REVOKED",
                        "Reason": "",
                    }
                )
