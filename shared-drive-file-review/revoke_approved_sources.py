...
def revoke_approved_sources():
    ...
    try:
        with open(APPROVED_CSV_FILE, mode='r', encoding='utf-8') as infile, \
             open(EVIDENCE_CSV_FILE, mode='w', newline='', encoding='utf-8') as outfile:
            
            reader = csv.DictReader(infile)
            writer = csv.DictWriter(outfile, fieldnames=['Timestamp', 'Source Name', 'Source ID', 'Target Email', 'Action'])
            writer.writeheader()
            ...
            for row in reader:
                ...
                try:
                    ...
                    if not perm_id_to_delete:
                        print(f" -> SKIP: Active permission not found. Already removed.")
                        continue
