...
def run_verification_audit():
    ...
    with open(OUTPUT_FILE, mode='w', newline='', encoding='utf-8') as f:
        ...
        writer.writerow(['Flagged External', 'Shared Drive Name', 'File Name', 'Modified Date', 'Folder Path', 'Permission Type', 'Role', 'Email/Domain', 'File ID', 'Link'])
        ...
        while True:
            ...
            for item in results.get('files', []):
                ...
                # 3. Fetch Permissions
                try:
                    perms = service.permissions().list(fileId=file_id, supportsAllDrives=True, fields="permissions(type, role, emailAddress, domain)").execute().get('permissions', [])
                except:
                    writer.writerow(['UNKNOWN', current_drive_name, item.get('name'), item.get('modifiedTime'), folder_path, 'ACCESS_DENIED', 'N/A', 'N/A', file_id, item.get('webViewLink')])
                    continue
                ...
                for p in perms:
                    ...
                    writer.writerow([is_external, current_drive_name, item.get('name'), item.get('modifiedTime'), folder_path, p.get('type'), p.get('role'), email, file_id, item.get('webViewLink')])
