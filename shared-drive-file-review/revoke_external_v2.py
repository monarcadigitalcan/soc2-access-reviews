...
def revoke_external_access():
    ...
    with open(INPUT_CSV, mode='r', encoding='utf-8') as infile, \
        ...
        writer = csv.DictWriter(outfile, fieldnames=[
            'Timestamp', 'File Name', 'File ID', 'Email/Domain',
            'Permission Type', 'Role', 'Action', 'Reason'
        ])
        writer.writeheader()
        outfile.flush()
        ...
        for row in reader:
            ...
            if remove != 'Y':
                stats['skipped'] += 1
                writer.writerow({
                    'Timestamp': timestamp, 'File Name': file_name,
                    'File ID': file_id, 'Email/Domain': email_domain,
                    'Permission Type': perm_type, 'Role': role,
                    'Action': 'SKIPPED', 'Reason': 'Not marked for removal'
                })
                outfile.flush()
                continue
            ...
            if perm_type == 'ACCESS_DENIED':
                ...
                writer.writerow({
                    ...
                    'File ID': file_id, 'Email/Domain': email_domain,
                    'Permission Type': perm_type, 'Role': role,
                    'Action': 'SKIPPED', 'Reason': 'ACCESS_DENIED — cannot read permissions'
                })
                outfile.flush()
                continue
            ...
            if role == 'owner':
                ...
                writer.writerow({
                    ...
                    'File ID': file_id, 'Email/Domain': email_domain,
                    'Permission Type': perm_type, 'Role': role,
                    'Action': 'SKIPPED', 'Reason': 'Owner permission — cannot be revoked via API'
                })
                outfile.flush()
                continue
            ...
            if not file_id:
                ...
                writer.writerow({
                    'Timestamp': timestamp, 'File Name': file_name,
                    'File ID': '', 'Email/Domain': email_domain,
                    'Permission Type': perm_type, 'Role': role,
                    'Action': 'SKIPPED', 'Reason': 'Missing File ID'
                })
                outfile.flush()
                continue
            ...
            try:
                results = service.permissions().list(
                    fileId=file_id,
                    fields="permissions(id, type, emailAddress, domain)",
            ...
            if stats['total'] % 50 == 0:
                print(f"[PROGRESS] {stats['total']}/891 processed | {stats['revoked']} revoked | {stats['skipped']} skipped | {stats['errors']} errors", flush=True)
